...
{	
	...
	{
		std::cerr << RED;
		std::cerr << executor.getName() << " executes " << this->_name << " fail!!!\n";
		std::cerr << F_NONE;
		throw Form::GradeTooLowException("Bureaucrat grade is too low, not execute(");
	}
	else
	{
		/* <bureaucrat> executes <form> */
		std::cout << GREEN;
		std::cout << executor.getName() << " executes " << this->_name << " successfully!!!\n";
		std::cout << F_NONE;
	}
}
